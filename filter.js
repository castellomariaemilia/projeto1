            ...
            for (let i = 0; i < docs.length; i++) {
                if(FIInstance){
                    for(let j = 0; j < FIInstance.length; j++){
                        for(let k = 0; k < docs[i].FI_x0020_Instance_x002C__x0020_Tool_x002C__x0020_Technology.length; k++){
                            if((docs[i].FI_x0020_Instance_x002C__x0020_Tool_x002C__x0020_Technology[k].toLowerCase()) === (FIInstance[j].value.toLowerCase()))
                            result1 = true;
                            if(result1)
                            break;
                        }
                ...
                if(FITeam) {
                    for(let j = 0; j < FITeam.length; j++){
                        if((docs[i].FI_x0020_Team_x002C__x0020_Process_x002C__x0020_Script_x0020_Group.toLowerCase()) === (FITeam[j].value.toLowerCase()))
                            result2 = true;
                            if(result2)
                            break;
                    }
